...
def down_pic(pic_urls: List[str], max_download_images: int) -> None:
    ...
    for i, pic_url in enumerate(pic_urls):
        try:
            ...
            image_output_path = './images/' + str(i) + '.jpg' #根据需求更改图片存储地址，图像名从0.jpg开始
            with open(image_output_path, 'wb') as f:
                f.write(pic.content)
                print('成功下载第%s张图片: %s' % (str(i), str(pic_url)))
        except RequestException as e:
            # 处理与请求相关的异常
            print('下载第%s张图片时遇到网络问题或无效URL: %s' % (str(i + 1), str(pic_url)))
            print(e)
        except IOError as e:
            # 处理输入输出相关的异常
            print('下载第%s张图片时发生文件操作错误: %s' % (str(i + 1), str(pic_url)))
            print(e)
        except Exception as e:
            # 捕获并处理其他所有未预料到的异常
            print('下载第%s张图片时发生未知错误: %s' % (str(i + 1), str(pic_url)))
            print(e)
        finally:
            # 可以在这里放一些无论是否异常都会执行的操作
            continue
